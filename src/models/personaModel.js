...
class Persona {
  ...
  // Actualiza una persona en Redis
  static async actualizar(dni, datosActualizados) {
    const personaExistente = await client.get(dni);
    if (!personaExistente) return null;
    
    const personaActualizada = { ...JSON.parse(personaExistente), ...datosActualizados };
    await client.set(dni, JSON.stringify(personaActualizada));
    return new Persona(personaActualizada);
  }

  // Elimina una persona de Redis
  static async eliminar(dni) {
    await client.del(dni);
  }
